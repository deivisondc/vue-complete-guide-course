<template>
  <ul>
    <li :class="{ active: modelValue === 'poor' }">
      <button
        :class="{ active: modelValue === 'poor' }"
        type="button"
        @click="activate('poor')"
      >
        Poor
      </button>
    </li>
    <li :class="{ active: modelValue === 'average' }">
      <button
        :class="{ active: modelValue === 'average' }"
        type="button"
        @click="activate('average')"
      >
        Average
      </button>
    </li>
    <li :class="{ active: modelValue === 'great' }">
      <button
        :class="{ active: modelValue === 'great' }"
        type="button"
        @click="activate('great')"
      >
        Great
      </button>
    </li>
  </ul>
</template>

<script>
export default {
  props: ['modelValue'],
  emits: ['update:modelValue'],
  // this wouldn't react with changes. It gets the value the first time only
  // data() {
  //   return {
  //     activeOption: this.modelValue,
  //   };
  // },

  // With computed, it will reflect the changes in the UI,
  // but in this case we could use the modelValue directly instead of activeOption
  // computed: {
  //   activeOption() {
  //     return this.modelValue;
  //   },
  // },
  methods: {
    activate(option) {
      // this.activeOption = option;
      this.$emit('update:modelValue', option);
    },
  },
};
</script>

<style scoped>
.active {
  border-color: #a00078;
}
.active button {
  color: #a00078;
}

ul {
  list-style: none;
  margin: 0.5rem 0;
  padding: 0;
  display: flex;
}

li {
  margin: 0 1rem;
  border: 1px solid #ccc;
  padding: 1rem;
  display: flex;
  justify-content: center;
  align-items: center;
}

button {
  font: inherit;
  border: none;
  background-color: transparent;
  cursor: pointer;
}
</style>
